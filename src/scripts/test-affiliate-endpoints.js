...
async function testEndpoints() {
  ...
  try {
    ...
    try {
      const linksResponse = await axios.get('http://localhost:3001/api/affiliate-links/startup/test-startup-id', { headers });
    ...
    if (firstLinkId) {
      ...
      try {
        const clicksResponse = await axios.get(`http://localhost:3001/api/affiliate-clicks/${firstLinkId}`, { headers });
