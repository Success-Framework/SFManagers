...
const LoginPage: React.FC = () => {
  ...
  // Critical: Check token IMMEDIATELY on component mount
  // before rendering anything
  useEffect(() => {
    const token = localStorage.getItem('token');
    if (token) {
      console.log('LoginPage: Token found - redirecting without rendering');
      setHasToken(true);
      navigate('/my-startups');
    }
  }, [navigate]);
  
  // If token exists, don't render anything at all
  if (hasToken) {
    return null;
  }
