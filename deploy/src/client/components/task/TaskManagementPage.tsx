...
const TaskManagementPage: React.FC = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (user?.id === startupResponse.data.ownerId) {
          setIsStartupOwner(true);
        }
        
        // Fetch opportunities for this startup
        const opportunitiesResponse = await axios.get(`/api/opportunities/startup/${startupId}`, {
          headers: { 'x-auth-token': token }
        });
        setOpportunities(opportunitiesResponse.data);
        
        // Fetch leads for this startup
        const leadsResponse = await axios.get(`/api/leads/startup/${startupId}`, {
          headers: { 'x-auth-token': token }
        });
  ...
  const isTabVisible = (tabId: string): boolean => {
    // Admin and Founder can access all tabs
    if (isAdmin || isStartupOwner) {
      return true;
    }

    // Manager can access specific tabs
    if (userRole?.toLowerCase().includes('manager')) {
      const managerTabs = [
        'tasks', 'addTask', 'taskAnalytics', 
        'analytics', 'meetings', 'opportunities', 'calendar',
        'affiliateAnalytics'
      ];
      return managerTabs.includes(tabId);
    }

    // Employee - Tech and Design
    if (userRole?.toLowerCase().includes('tech') || userRole?.toLowerCase().includes('design')) {
      const techDesignTabs = [
        'tasks', 'addTask', 'meetings', 'opportunities', 'calendar'
      ];
      return techDesignTabs.includes(tabId);
    }

    // Employee - Sales, Marketing and Operations
    if (userRole?.toLowerCase().includes('sales') || 
        userRole?.toLowerCase().includes('marketing') || 
        userRole?.toLowerCase().includes('operations')) {
      const salesMarketingOpsTabs = [
        'tasks', 'addTask', 'meetings', 'opportunities', 'calendar'
      ];
      return salesMarketingOpsTabs.includes(tabId);
    }

    // Default - show only basic tabs
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    if (!taskForm.title || !taskForm.statusId || !startupId) {
      setError('Title, status and startup are required');
      return;
    }
  ...
  const openNewTaskModal = () => {
    ...
    setTaskForm({
      ...
      priority: 'medium',
      dueDate: new Date().toISOString().split('T')[0],
      statusId: statuses[0].id, // Set initial status
      assigneeIds: [] as string[] // Explicitly include assigneeIds
    });
  ...
  const handleStopTimer = async (note: string) => {
    ...
    try {
      const response = await fetch(`/api/tasks/${currentTask.id}/timer/stop`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      // Update the task in the local tasks list
      setTasks(currentTasks => 
        currentTasks.map(task => task.id === currentTask.id ? updatedTask : task)
      );
      ...
      // Refresh time logs by triggering a re-render of TimeLogs component
      // We'll do this by updating the key of the TimeLogs component
  ...
  // Render affiliate links section
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'leadsAnalytics':
        // Only show to founders, admins, and managers
        if (!isStartupOwner && !isAdmin && userRole !== 'Manager') {
          return (
            <div className="alert alert-warning">
              <h4 className="alert-heading">Access Restricted</h4>
              <p>You do not have permission to view Leads Analytics. This feature is available to startup founders, administrators, and managers only.</p>
            </div>
          );
        }
      ...
      <div className="d-flex">
        ...
        <div className={`sidebar bg-dark text-white ${sidebarCollapsed ? 'collapsed' : ''}`} 
          ...
          <ul className="nav flex-column">
            {sidebarTabs.map(tab => isTabVisible(tab.id) && (
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                ...
                <div className="mb-3">
                  ...
                  <TimeTracker 
                    task={currentTask} 
                    onTimerStart={() => handleStartTimer(currentTask.id)} 
                    onTimerPause={() => handlePauseTimer(currentTask.id)}
              ...
              <div className="modal-body">
                <div className="row">
                  <div className="col-md-6">
                    ...
                    <p><strong>Created:</strong> {new Date(currentLead.createdAt).toLocaleString()}</p>
                    <p><strong>Last Updated:</strong> {new Date(currentLead.updatedAt).toLocaleString()}</p>
                  </div>
                  <div className="col-md-6">
                    <h6>Notes</h6>
