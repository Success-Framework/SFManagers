...
router.post('/', authMiddleware, upload.fields([
    ...
    try {
        ...
        const startup = yield prisma.startup.create({
            data: {
                name,
                details: description,
...
router.get('/', (_req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const startups = yield prisma.startup.findMany({
            include: {
                roles: true,
                owner: {
                    select: {
...
router.get('/my-startups', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const userId = req.user.id;
        const startups = yield prisma.startup.findMany({
            where: {
...
router.get('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const transformedStartup = Object.assign(Object.assign({}, startup), { roles: startup.roles.map(role => {
                // Get the first assigned user if any
                const assignedUser = role.users && role.users.length > 0
                    ? role.users[0].user
                    : undefined;
                return Object.assign(Object.assign({}, role), { assignedUser });
            }) });
...
router.put('/:id', authMiddleware, upload.fields([
    ...
    try {
        ...
        // Update startup
        const updatedStartup = yield prisma.startup.update({
            where: { id },
            data: updateData,
            include: {
                roles: true,
...
router.get('/:startupId/members', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!isOwner) {
            const isMember = yield prisma.userRole.findFirst({
                where: {
                    userId,
                    role: {
                        startupId
        ...
        const roleMembers = yield prisma.user.findMany({
            where: {
                joinedRoles: {
                    some: {
                        role: {
        ...
        const members = [owner, ...roleMembers].filter((member, index, self) => index === self.findIndex(m => m.id === member.id));
        res.json(members);
    }
    catch (err) {
...
router.get('/:startupId/members-with-roles', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { startupId } = req.params;
        const userId = req.user.id;
        // Check if startup exists
...
router.put('/:startupId/users/:userId/role', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!isOwner) {
            const requestingUserRole = yield prisma.userRole.findFirst({
                where: {
...
router.post('/:id/roles', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        ...
        const createdRoles = yield Promise.all(roles.map(role => prisma.role.create({
            data: {
                title: role.title,
                roleType: role.roleType,
        ...
        const updatedStartup = yield prisma.startup.findUnique({
            where: { id },
            include: {
                roles: {
                    include: {
        ...
        // Transform the data to include assignedUser directly on roles
        const transformedStartup = Object.assign(Object.assign({}, updatedStartup), { roles: updatedStartup.roles.map(role => {
                // Get the first assigned user if any
                const assignedUser = role.users && role.users.length > 0
                    ? role.users[0].user
                    : undefined;
...
router.put('/roles/:roleId', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const assignedUser = updatedRole.users && updatedRole.users.length > 0
            ...
            : undefined;
        const transformedRole = Object.assign(Object.assign({}, updatedRole), { assignedUser });
        return res.json({
...
router.delete('/roles/:roleId', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { roleId } = req.params;
        const userId = req.user.id;
        // Find the role and check if it exists
        ...
        yield prisma.joinRequest.deleteMany({
            where: { roleId }
        });
        // Delete the role
...
router.get('/:startupId/roles', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { startupId } = req.params;
        const userId = req.user.id;
        // Check if startup exists
